...
namespace orbiterkep {
    ...
    TCLAP::ValueArg<double>      tofMinArg ("", "tof-min",  "minimum time of flight", false, 0.1, "0.1");
    TCLAP::ValueArg<double>      tofMaxArg ("", "tof-max",  "maximum time of flight", false, 7, "7");
    ...
    bool parse_parameters(Parameters * param, int argc, char ** argv) {
      ...
      try {
        ...
        param->mutable_tof()->set_lb( tofMinArg.getValue() );
        param->mutable_tof()->set_ub( tofMaxArg.getValue() );

        param->mutable_vinf()->set_lb( 0.1 );
        param->mutable_vinf()->set_ub( vinfMaxArg.getValue() );
