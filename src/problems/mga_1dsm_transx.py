...
from PyGMO.problem import transx_problem
...
class mga_1dsm_transx(transx_problem):
    ...
    def __init__(self, seq=[jpl_lp('earth'), jpl_lp('venus'), jpl_lp('earth')], 
                 dep_altitude = 300, arr_altitude = 300, circularize = False,
                 t0=[epoch(0), epoch(1000)], tof=[1.0, 5.0], vinf=[0.5, 2.5], add_vinf_dep=False, add_vinf_arr=True, multi_objective=False):
        """
        ...
        super(mga_1dsm_transx, self).__init__(seq, dep_altitude, arr_altitude, circularize, dim, obj_dim)
    ...
    def calc_objective(self, x, should_print = False):
        ...
        if self.__add_vinf_dep:
            DV[0] += self.burn_cost(self.seq[0], Vinf, arr = False, circ = False)
            if should_print:
        ...
        if self.__add_vinf_arr:
            ...
            DV[-1] = self.burn_cost(self.seq[-1], Vexc_arr, arr = True, circ = self.circularize)
            if should_print:
